...
AI 驱动的仙侠连载小说 —— 全书 115 章，零外部依赖。

**阅读地址**：[lycbee.github.io/jianyu-jianghu](https://lycbee.github.io/jianyu-jianghu/)

## 核心机制

- **全书 115 章**：三幕结构（1-30 困兽之斗 / 31-70 问剑天下 / 71-115 剑雨终章），已于 2026-05-19 完结
- **每日自动生成**（已停用）：定时生成连载章节（约 2500 字/章），写作模型（temp 0.85）+ 编辑模型（temp 0.3）双模型协作
...
- **自动部署**（已停用）：生成后构建静态 HTML → GitHub Pages，全程自动化
...
# 5. 生成章节（仅调试/补写，全书已完结，超过115章自动停止）
...
- **CI 已停用**：全书完结后，GitHub Actions 定时任务已取消，仅保留手动触发入口。
