...
def main():
    ...
    if results:
        chapter_nums = {r[0] for r in results}
        ...
        for act_boundary in [30, 70]:
            if act_boundary in chapter_nums:
                next_act = 2 if act_boundary == 30 else 3
                print(f"\n{'='*60}")
                print(f"📖 第{act_boundary}章幕结束 — 深度回顾与规划")
                print(f"{'='*60}")
                try:
